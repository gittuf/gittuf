...
func TestAddHook(t *testing.T) {
	...
	t.Run("valid pre-commit hook", func(t *testing.T) {
		...
		err = r.AddHook(testCtx, rootSigner, hookStage, hookName, hookPath, environment, modules, principals, true, trustpolicyopts.WithRSLEntry())
		assert.Nil(t, err)
		...
		preCommitHook := tufv01.Hook{
			...
			Hashes:       map[string]string{"sha1": hookHash.String(), "sha256": sha256HashSum},
			Environment:  tuf.HookEnvironmentLua,
			Modules:      []string{},
		}
...
func TestRemoveHook(t *testing.T) {
	...
	t.Run("valid pre-commit hook", func(t *testing.T) {
		...
		// Add hook
		if err := r.AddHook(testCtx, rootSigner, hookStage, hookName, hookPath, environment, modules, principals, true, trustpolicyopts.WithRSLEntry()); err != nil {
			t.Fatal(err)
		}
		err = r.StagePolicy(testCtx, "", true, false)
		require.Nil(t, err)
