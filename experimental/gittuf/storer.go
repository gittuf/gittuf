...
// StorerBackendEnvKey is the only way to select a backend. Every backend
// other than the default additionally requires developer mode.
...
const experimentalStorerWarning = "the go-git storage backend is EXPERIMENTAL: results are not covered by gittuf's compatibility guarantees and may differ from the default backend; unset " + StorerBackendEnvKey + " to disable"
...
// Outside developer mode it always resolves to the default backend.
func (s *storerSelection) resolve() StorerBackend {
	...
	if backend != StorerBackendBinary && !dev.InDevMode() {
		backend = StorerBackendBinary
	}
...
// Every backend other than the default requires developer mode, so callers get
// dev.ErrNotInDevMode rather than a silent downgrade. An unrecognised backend
// is rejected, so what StorerBackendInUse reports is what repositories load
// with.
func SetStorerBackend(backend StorerBackend) error {
	backend, err := ParseStorerBackend(string(backend))
	if err != nil {
		return err
	}
...
// SetStorerTrace enables per method counts and timings for subsequently
// loaded repositories, read back with StorerTraceReport. A repository keeps
// the trace it was given when it was loaded. Disabling discards any trace
// already collected.
...
func clearActiveTrace() {
	...
	activeTrace.trace = nil
	activeTrace.repo = nil
	activeTrace.backend = ""
}
