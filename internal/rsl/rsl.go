...
func NewPropagationEntry(refName string, targetID gitinterface.Hash, upstreamRepository string, upstreamEntryID gitinterface.Hash) *PropagationEntry {
	return &PropagationEntry{RefName: refName, TargetID: targetID, UpstreamRepository: upstreamRepository, UpstreamEntryID: upstreamEntryID}
}
...
func PropagateChanges(downstreamRepo, upstreamRepo *gitinterface.Repository, details []*PropagationDetails, sign bool) error {
	// We assume here that downstreamRepo and upstreamRepo have their gittuf
	// refs already synced.

	for _, detail := range details {
		latestUpstreamEntry, _, err := GetLatestReferenceEntry(upstreamRepo, ForReference(detail.UpstreamRef), IsUnskipped())
		if err != nil {
			if !errors.Is(err, ErrRSLEntryNotFound) {
				return err
			}
		...
		currentRefTip, err := downstreamRepo.GetReference(detail.DownstreamRef)
		if err != nil {
		...
		currentPathTreeID, err := downstreamRepo.GetPathIDInTree(detail.DownstreamPath, currentTreeID)
		if err != nil {
		...
		// TODO: batch this up?
		refCommitMap, err := downstreamRepo.PropagateUpstreamRepositoryContents(upstreamRepo, latestUpstreamEntry.TargetID, map[string]string{detail.DownstreamRef: detail.DownstreamPath})
		if err != nil {
			return err
		}

		for ref, commitID := range refCommitMap {
			if err := NewPropagationEntry(ref, commitID, detail.UpstreamRepository, latestUpstreamEntry.ID).Commit(downstreamRepo, sign); err != nil {
				return err
			}
		}
	}
