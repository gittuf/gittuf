...
func TestAddDelegation(t *testing.T) {
	...
	t.Run("valid rule / delegation name", func(t *testing.T) {
		...
		err = r.AddDelegation(testCtx, targetsSigner, policy.TargetsRoleName, ruleName, authorizedKeyBytes, rulePatterns, 1, []tuf.Role{{KeyIDs: []string{targetsPubKey.KeyID}, Threshold: 1}}, false)
		assert.Nil(t, err)
		...
		assert.Contains(t, targetsMetadata.Delegations.Roles, tuf.Delegation{
			...
			Terminating: false,
			MinRoles:    1,
			Roles:       []tuf.Role{{KeyIDs: []string{targetsPubKey.KeyID}, Threshold: 1}},
		})
		assert.Contains(t, targetsMetadata.Delegations.Roles, policy.AllowRule())
	})
	...
	t.Run("invalid rule name", func(t *testing.T) {
		...
		err := r.AddDelegation(testCtx, targetsSigner, policy.TargetsRoleName, policy.RootRoleName, nil, nil, 1, nil, false)
		assert.ErrorIs(t, err, ErrInvalidPolicyName)
	})
}
...
func TestUpdateDelegation(t *testing.T) {
	...
	err = r.UpdateDelegation(testCtx, targetsSigner, policy.TargetsRoleName, "protect-main", []*tuf.Key{gpgKey, targetsKey}, []string{"git:refs/heads/main"}, 1, []tuf.Role{{KeyIDs: []string{gpgKey.KeyID, targetsKey.KeyID}, Threshold: 1}}, false)
	assert.Nil(t, err)
	...
	assert.Contains(t, targetsMetadata.Delegations.Roles, tuf.Delegation{
		...
		Terminating: false,
		MinRoles:    1,
		Roles:       []tuf.Role{{KeyIDs: []string{gpgKey.KeyID, targetsKey.KeyID}, Threshold: 1}},
	})
}
