...
func getGitHubClient() (*github.Client, error) {
	...
	if baseURL != "" {
		...
		apiEndpoint := fmt.Sprintf("%s/%s/%s/", baseURL, "api", "v3")
		uploadEndpoint := fmt.Sprintf("%s/%s/%s/", baseURL, "api", "uploads")

		var err error
		githubClient, err = githubClient.WithEnterpriseURLs(apiEndpoint, uploadEndpoint)
		if err != nil {
			return nil, err
		}
	}
