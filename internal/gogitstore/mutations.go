...
// CreateSubtreeFromUpstreamRepository needs both repositories at once, so it
// stays on the git binary whatever backend is selected.
func (s *Storer) CreateSubtreeFromUpstreamRepository(upstream *gitinterface.Repository, upstreamCommitID githash.Hash, upstreamPath, localRef, localPath string) (githash.Hash, error) {
	defer s.record("CreateSubtreeFromUpstreamRepository", time.Now(), true)
	defer s.invalidate()
	return s.Repository.CreateSubtreeFromUpstreamRepository(upstream, upstreamCommitID, upstreamPath, localRef, localPath)
}
