...
// beginRead opens the cached handle and returns a function recording the call.
// Callers defer it immediately, so the trace covers opening the handle and
// still counts a call whose open failed.
func (s *Storer) beginRead(method string) (*gogit.Repository, func(), error) {
	start := time.Now()

	repo, err := s.fast()
	delegated := err == nil && repo == nil

	return repo, func() { s.record(method, start, delegated) }, err
}
...
func (s *Storer) ReadBlob(blobID githash.Hash) ([]byte, error) {
	repo, done, err := s.beginRead("ReadBlob")
	defer done()
	if err != nil {
		return nil, err
	}
...
func (s *Storer) GetCommitMessage(commitID githash.Hash) (string, error) {
	repo, done, err := s.beginRead("GetCommitMessage")
	defer done()
	if err != nil {
		return "", err
	}
...
func (s *Storer) GetCommitTreeID(commitID githash.Hash) (githash.Hash, error) {
	repo, done, err := s.beginRead("GetCommitTreeID")
	defer done()
	if err != nil {
		return nil, err
	}
...
func (s *Storer) GetCommitParentIDs(commitID githash.Hash) ([]githash.Hash, error) {
	repo, done, err := s.beginRead("GetCommitParentIDs")
	defer done()
	if err != nil {
		return nil, err
	}
...
func (s *Storer) GetEntriesInTree(treeID githash.Hash) ([]gitstore.TreeEntry, error) {
	repo, done, err := s.beginRead("GetEntriesInTree")
	defer done()
	if err != nil {
		return nil, err
	}
...
func (s *Storer) GetAllFilesInTree(treeID githash.Hash) (map[string]githash.Hash, error) {
	repo, done, err := s.beginRead("GetAllFilesInTree")
	defer done()
	if err != nil {
		return nil, err
	}
...
func (s *Storer) GetPathIDInTree(treeID githash.Hash, treePath string) (githash.Hash, error) {
	repo, done, err := s.beginRead("GetPathIDInTree")
	defer done()
	if err != nil {
		return nil, err
	}
...
func (s *Storer) GetReference(refName string) (githash.Hash, error) {
	...
	// Decide delegation before opening a handle, so a revision expression does
	// not pay the open cost.
	if !isFullReferenceName(refName) {
		defer s.record("GetReference", start, true)
		return s.Repository.GetReference(refName)
	}

	repo, err := s.fast()
	delegated := err == nil && repo == nil
	defer s.record("GetReference", start, delegated)
...
func (s *Storer) GetTagTarget(tagID githash.Hash) (githash.Hash, error) {
	repo, done, err := s.beginRead("GetTagTarget")
	defer done()
	if err != nil {
		return nil, err
	}
...
func (s *Storer) GetObjectSignature(objectID githash.Hash) ([]byte, []byte, error) {
	repo, done, err := s.beginRead("GetObjectSignature")
	defer done()
	if err != nil {
		return nil, nil, err
	}
...
// isEmptyTree reports whether treeID is the repository's empty-tree ID. The
// other format's ID does not count: the binary backend rejects it, so
// accepting it here would let malformed input succeed.
func (s *Storer) isEmptyTree(treeID githash.Hash) bool {
	emptyTree := emptyTreeSHA1
	if s.GetObjectFormat() == gitinterface.ObjectFormatSHA256 {
		emptyTree = emptyTreeSHA256
	}

	return treeID.String() == emptyTree
}
