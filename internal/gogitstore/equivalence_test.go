...
func TestGetEntriesInTree(t *testing.T) {
	...
	for _, objectFormat := range objectFormats {
		t.Run(string(objectFormat), func(t *testing.T) {
			...
			t.Run("the other format's empty tree ID is rejected by both backends", func(t *testing.T) {
				foreignEmptyTree := otherFormatEmptyTreeID(t, objectFormat)

				_, wantErr := binary.GetEntriesInTree(foreignEmptyTree)
				require.Error(t, wantErr, "the git binary rejects a cross-format ID")

				_, gotErr := fast.GetEntriesInTree(foreignEmptyTree)
				assert.Error(t, gotErr, "the empty tree special case must not accept the other format's ID")
			})
