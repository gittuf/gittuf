...
// first use.
func getTrustedSigstoreRootPools() (*x509.CertPool, *x509.CertPool, error) {
	trustedRootOnce.Do(func() {
		cachedTrustedRoot, cachedRootPool, cachedIntermediatePool, cachedTrustedRootErr = fetchTrustedSigstoreRootPools()
	})

	return cachedRootPool, cachedIntermediatePool, cachedTrustedRootErr
}

// fetchTrustedSigstoreRootPools fetches the Sigstore trusted root over TUF and builds
// the root and intermediate certificate pools from it. It performs no caching
// of its own.
func fetchTrustedSigstoreRootPools() (*sigstoreroot.TrustedRoot, *x509.CertPool, *x509.CertPool, error) {
...
func verifyGitsignSignature(ctx context.Context, key *signerverifier.SSLibKey, data, signature []byte, rekorURL string) error {
	...
	if sigstoreRootFilePath == "" {
		root, intermediate, err := getTrustedSigstoreRootPools()
		if err != nil {
