...
func TestVerifier(t *testing.T) {
	...
	approvalAttestationWithOneApproverStmt, err := attestations.NewGitHubPullRequestApprovalAttestation("refs/heads/main", gitinterface.ZeroHash.String(), gitinterface.ZeroHash.String(), []*tuf.Key{targetsPubKey})
	if err != nil {
		t.Fatal(err)
	}
	approvalAttestationWithOneApprover, err := dsse.CreateEnvelope(approvalAttestationWithOneApproverStmt)
	if err != nil {
		t.Fatal(err)
	}
	approvalAttestationWithOneApprover, err = dsse.SignEnvelope(testCtx, approvalAttestationWithOneApprover, rootSigner)
	if err != nil {
		t.Fatal(err)
	}

	approvalAttestationWithTwoApproversStmt, err := attestations.NewGitHubPullRequestApprovalAttestation("refs/heads/main", gitinterface.ZeroHash.String(), gitinterface.ZeroHash.String(), []*tuf.Key{targetsPubKey, rootPubKey})
	if err != nil {
		t.Fatal(err)
	}
	approvalAttestationWithTwoApprovers, err := dsse.CreateEnvelope(approvalAttestationWithTwoApproversStmt)
	if err != nil {
		t.Fatal(err)
	}
	approvalAttestationWithTwoApprovers, err = dsse.SignEnvelope(testCtx, approvalAttestationWithTwoApprovers, rootSigner)
	if err != nil {
		t.Fatal(err)
	}
	...
	tests := map[string]struct {
		...
		githubApprovalAttestation *sslibdsse.Envelope
		...
		"commit, GitHub approval attestation, valid keys, threshold 2": {
			keys:                      []*tuf.Key{gpgKey, targetsPubKey},
			threshold:                 2,
			gitObjectID:               commitID,
			githubAppKey:              rootPubKey,
			githubApprovalAttestation: approvalAttestationWithOneApprover,
		},
		"commit, GitHub approval attestation, valid keys, threshold 3": {
			keys:                      []*tuf.Key{gpgKey, targetsPubKey, rootPubKey},
			threshold:                 3,
			gitObjectID:               commitID,
			githubAppKey:              rootPubKey,
			githubApprovalAttestation: approvalAttestationWithTwoApprovers,
		},
		"commit, GitHub approval attestation and reference authorization, valid keys, threshold 3": {
			keys:                      []*tuf.Key{gpgKey, targetsPubKey, rootPubKey},
			threshold:                 3,
			gitObjectID:               commitID,
			attestation:               attestation, // signed by rootSigner
			githubAppKey:              rootPubKey,
			githubApprovalAttestation: approvalAttestationWithTwoApprovers, // contains targetsPubKey as approver
		},
		"commit, GitHub approval attestation, invalid approvals, threshold 3": {
			keys:                      []*tuf.Key{gpgKey, targetsPubKey, rootPubKey},
			threshold:                 3,
			gitObjectID:               commitID,
			githubAppKey:              rootPubKey,
	...
	for name, test := range tests {
		...
		if test.githubAppKey != nil {
			verifier.githubAppKey = test.githubAppKey
			verifier.githubAppApprovalTrusted = true
		}

		err := verifier.Verify(testCtx, test.gitObjectID, test.attestation, test.githubApprovalAttestation)
