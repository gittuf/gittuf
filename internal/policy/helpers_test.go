...
func createTestStateWithPolicy(t *testing.T) *State {
	...
	targetsMetadata, err = AddDelegation(targetsMetadata, "protect-main", []*tuf.Key{gpgKey}, []string{"git:refs/heads/main"}, 1, []tuf.Role{{KeyIDs: []string{gpgKey.KeyID}, Threshold: 1}})
	if err != nil {
		t.Fatal(err)
	}
	// Add a file protection rule. When used with common.AddNTestCommitsToSpecifiedRef, we have files with names 1, 2, 3,...n.
	targetsMetadata, err = AddDelegation(targetsMetadata, "protect-files-1-and-2", []*tuf.Key{gpgKey}, []string{"file:1", "file:2"}, 1, []tuf.Role{{KeyIDs: []string{gpgKey.KeyID}, Threshold: 1}})
	if err != nil {
		t.Fatal(err)
	}
...
func createTestStateWithDelegatedPolicies(t *testing.T) *State {
	...
	targetsMetadata, err = AddDelegation(targetsMetadata, "1", []*tuf.Key{key}, []string{"file:1/*"}, 1, []tuf.Role{{KeyIDs: []string{key.KeyID}, Threshold: 1}})
	if err != nil {
		t.Fatal(err)
	}

	targetsMetadata, err = AddDelegation(targetsMetadata, "2", []*tuf.Key{key}, []string{"file:2/*"}, 1, []tuf.Role{{KeyIDs: []string{key.KeyID}, Threshold: 1}})
	if err != nil {
		t.Fatal(err)
	}
	...
	delegation1Metadata, err = AddDelegation(delegation1Metadata, "3", []*tuf.Key{gpgKey}, []string{"file:1/subpath1/*"}, 1, []tuf.Role{{KeyIDs: []string{gpgKey.KeyID}, Threshold: 1}})
	if err != nil {
		t.Fatal(err)
	}

	delegation1Metadata, err = AddDelegation(delegation1Metadata, "4", []*tuf.Key{gpgKey}, []string{"file:1/subpath2/*"}, 1, []tuf.Role{{KeyIDs: []string{gpgKey.KeyID}, Threshold: 1}})
	if err != nil {
		t.Fatal(err)
	}
...
func createTestStateWithThresholdPolicy(t *testing.T) *State {
	...
	targetsMetadata, err = UpdateDelegation(targetsMetadata, "protect-main", []*tuf.Key{gpgKey, approverKey}, []string{"git:refs/heads/main"}, 1, []tuf.Role{{KeyIDs: []string{gpgKey.KeyID, approverKey.KeyID}, Threshold: 2}})
	if err != nil {
		t.Fatal(err)
	}
...
func createTestStateWithTagPolicy(t *testing.T) *State {
	...
	targetsMetadata, err = AddDelegation(targetsMetadata, "protect-tags", []*tuf.Key{gpgKey}, []string{"git:refs/tags/*"}, 1, []tuf.Role{{KeyIDs: []string{gpgKey.KeyID}, Threshold: 1}})
	if err != nil {
		t.Fatal(err)
	}
	targetsEnv, err := dsse.CreateEnvelope(targetsMetadata)
...
func createTestStateWithTagPolicyForUnauthorizedTest(t *testing.T) *State {
	...
	targetsMetadata, err = AddDelegation(targetsMetadata, "protect-tags", []*tuf.Key{rootKey}, []string{"git:refs/tags/*"}, 1, []tuf.Role{{KeyIDs: []string{rootKey.KeyID}, Threshold: 1}})
	if err != nil {
		t.Fatal(err)
	}
	targetsEnv, err := dsse.CreateEnvelope(targetsMetadata)
