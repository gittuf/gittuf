...
func (o *options) Run(cmd *cobra.Command, _ []string) error {
	...
	if o.githubToken == "" {
		fmt.Println("NOTE: A GitHub token was not specified. gittuf will be setup such that only you will be the only authorized user to make and approve changes to the default branch.")
		fmt.Println("To allow the setup wizard to enable other users to make changes to the default branch, rerun the setup command with `--github-token` and your token.")
		fmt.Println()
	}
	...
	if err = repo.AddPrincipalToTargets(cmd.Context(), signer, policy.TargetsRoleName, []tuf.Principal{sigstorePerson}, true); err != nil {
		return err
	}
	...
	if o.githubToken != "" {
		...
		for _, username := range gitHubUsernames {
			...
			githubPersonKey, err := gittuf.LoadPublicKey(fmt.Sprintf("fulcio:%s::%s", user.GetEmail(), "https://github.com/login/oauth"))
			if err != nil {
				return err
			}
			...
			publicKeys[githubPersonKey.ID()] = githubPersonKey.(*tufv02.Key)
	...
	principalNames := []string{sigstorePerson.ID()}
	if o.githubToken != "" {
		principalNames = append(principalNames, gitHubUsernames...)
	}
