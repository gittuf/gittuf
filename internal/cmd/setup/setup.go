...
func (o *options) Run(cmd *cobra.Command, args []string) error {
	...
	case "maintainer":
		...
		if err != nil {
			if !errors.Is(err, svgit.ErrSigningKeyNotSpecified) {
				return err
			}
		...
		if err != nil {
			...
			fmt.Println("If you are NOT expecting this, press Ctrl+C NOW, and visit https://gittuf.dev/initialized.")
			if _, err = fmt.Scanln(); err != nil {
				return err
			}
		...
		if initializePolicy {
			...
			publicKey, err := gittuf.LoadPublicKey(fmt.Sprintf("%s:%s", "TODO", keyID))
			if err != nil {
				return err
			}
		...
		// Conclude
		fmt.Println("gittuf enabled successfully!")
...
// Determine the default branch of the repository. We try to find "main" or
// "master". If none exist, ask the user.
