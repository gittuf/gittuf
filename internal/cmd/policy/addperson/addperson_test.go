...
func TestAddPerson(t *testing.T) {
	...
	t.Run("invalid associated identity", func(t *testing.T) {
		tmpDir := t.TempDir()
		gitinterface.CreateTestGitRepository(t, tmpDir, false)
	...
	t.Run("success", func(t *testing.T) {
		...
		_, _, _, err = cmd.ExecuteCommandC(New(pOpts), "--person-ID", "jane.doe@example.com", "--public-key", newKeyPath+".pub", "--associated-identity", "github::jane-doe", "--custom", "department=engineering")
	...
	t.Run("success with RSL entry", func(t *testing.T) {
		...
		_, _, _, err = cmd.ExecuteCommandC(New(pOpts), "--person-ID", "jane.doe@example.com", "--public-key", newKeyPath+".pub")
		assert.NoError(t, err)
	})
	...
	t.Run("success with custom policy name", func(t *testing.T) {
		...
		require.NoError(t, err)
		require.NoError(t, repo.InitializeRoot(t.Context(), signer, false))
		require.NoError(t, repo.InitializeTargets(t.Context(), signer, "custom-policy", false))

		pOpts := &persistent.Options{
			SigningKey: keyPath,
		}
		_, _, _, err = cmd.ExecuteCommandC(New(pOpts), "--person-ID", "jane.doe@example.com", "--public-key", newKeyPath+".pub", "--policy-name", "custom-policy")
		assert.NoError(t, err)
	})
}
