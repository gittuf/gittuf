...
func (o *options) Run(cmd *cobra.Command, _ []string) error {
	...
	return repo.RemovePrincipalFromTargets(cmd.Context(), signer, o.policyName, []string{o.personID}, true)
}
...
func New(persistent *persistent.Options) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long:              `This command allows users to remove a person from the specified policy file. By default, the main policy file is selected. Note that the person's keys can be specified from disk, from the GPG keyring using the "gpg:<fingerprint>" format, or as a Sigstore identity as "fulcio:<identity>::<issuer>".`,
