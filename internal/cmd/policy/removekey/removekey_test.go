...
func TestRemoveKey(t *testing.T) {
	...
	t.Run("success", func(t *testing.T) {
		...
		_, _, _, err = cmd.ExecuteCommandC(command, "--public-key", newKey.ID())
		...
		_, ok := targetsMetadata.GetPrincipals()[newKey.ID()]
		assert.False(t, ok)
	})
	...
	t.Run("success with custom policy name", func(t *testing.T) {
		...
		command := New(&persistent.Options{SigningKey: keyPath, WithRSLEntry: true})
		_, _, _, err = cmd.ExecuteCommandC(command, "--policy-name", "custom-policy", "--public-key", newKey.ID())
		assert.NoError(t, err)
