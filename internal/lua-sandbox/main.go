...
import (
	...
	sandbox "github.com/gittuf/gittuf/internal/lua-sandbox/api"
...
func NewLuaEnvironment(allowedModules []string) (*lua.LState, error) {
	...
	lState, err := sandbox.RegisterAPIFunctions(lState, allowedModules)
	if err != nil {
		return nil, err
	}
...
func setInstructionQuota(l *lua.LState, quota int64) error {
	// Get the `set_instruction_quota` function from the Lua state
	fn := l.GetGlobal("set_instruction_quota")
	if fn == lua.LNil {
		return fmt.Errorf("set_instruction_quota function not found in Lua state")
	}

	// Push the quota integer onto the stack
	l.Push(fn)
	l.Push(lua.LNumber(quota))

	// Call the function with 1 argument and 0 return values
	if err := l.PCall(1, 0, nil); err != nil {
		return fmt.Errorf("error calling set_instruction_quota: %w", err)
	}
	return nil
}
