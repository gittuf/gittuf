...
type Attestations struct {
	...
	// `<repo-owner-username>-<repo-owner-id>` to another map with key
	// `<ref-path>/<commit-id>`, where `repo-owner-username` and `repo-owner-id`
	// identify the owner of the GitHub repository the attestation applies to,
	// `ref-path` is the absolute ref path, and `commit-id` is the ID of the
	// merged commit.
	githubPullRequestAttestations map[string]map[string]gitinterface.Hash
...
func LoadAttestationsForEntry(repo *gitinterface.Repository, entry rsl.ReferenceUpdaterEntry) (*Attestations, error) {
	...
	for name, blobID := range treeContents {
		...
		case strings.HasPrefix(name, githubPullRequestAttestationsTreeEntryName+"/"):
			relativePath := strings.TrimPrefix(name, githubPullRequestAttestationsTreeEntryName+"/")
			baseRepoInformation := strings.Split(relativePath, "/")[0]
			refAndCommitInformation := strings.TrimPrefix(relativePath, baseRepoInformation+"/")

			if attestations.githubPullRequestAttestations[baseRepoInformation] == nil {
				attestations.githubPullRequestAttestations[baseRepoInformation] = map[string]gitinterface.Hash{}
			}

			attestations.githubPullRequestAttestations[baseRepoInformation][refAndCommitInformation] = blobID
...
func (a *Attestations) Commit(repo *gitinterface.Repository, commitMessage string, createRSLEntry, signCommit bool) error {
	...
	for baseName, mapping := range a.githubPullRequestAttestations {
		for name, blobID := range mapping {
			allAttestations = append(allAttestations, gitinterface.NewEntryBlob(path.Join(githubPullRequestAttestationsTreeEntryName, baseName, name), blobID))
		}
	}
