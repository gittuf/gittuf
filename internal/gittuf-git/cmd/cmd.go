...
func Commit(ctx context.Context, gitArgs args.Args) error {
	if gitArgs.ChdirIdx > 0 {
		if err := os.Chdir(gitArgs.GlobalFlags[gitArgs.ChdirIdx]); err != nil {
			return err
		}
	...
	// TODO: Update once hook invocation flow in experimental/gittuf/hook.go is
	// decided
	...
	exitCodes, err := repo.InvokeHook(context.Background(), tuf.HookStagePreCommit, signer, policy.TargetsRoleName, attest)
	if err != nil {
		return err
	}
