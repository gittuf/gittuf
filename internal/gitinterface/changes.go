...
func (r *Repository) GetFilePathsChangedByCommit(commitID Hash) ([]string, error) {
	...
	if len(parentCommitIDs) == 0 {
		filePaths, err := r.executeGitCommandString("ls-tree", "--name-only", "-r", commitID.String())
		if err != nil {
	...
	if len(parentCommitIDs) > 1 {
		...
		stdOut, err := r.executeGitCommandString("diff-tree", "--no-commit-id", "--name-only", "-r", parentCommitIDs[len(parentCommitIDs)-1].String(), commitID.String())
		if err != nil {
		...
		for _, parentCommitID := range parentCommitIDs {
			stdOut, err := r.executeGitCommandString("diff-tree", "--no-commit-id", "--name-only", "-r", parentCommitID.String(), commitID.String())
			if err != nil {
	...
	stdOut, err := r.executeGitCommandString("diff-tree", "--no-commit-id", "--name-only", "-r", fmt.Sprintf("%s~1", commitID.String()), commitID.String())
	if err != nil {
		return nil, fmt.Errorf("unable to diff commit against parent: %w", err)
	}
	if stdOut == "" {
