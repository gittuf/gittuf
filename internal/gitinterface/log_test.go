...
func TestGetCommitsBetweenRange(t *testing.T) {
	...
	t.Run("Check range between commits 1 and 5", func(t *testing.T) {
		...
		assert.Nil(t, err)
		expectedCommits := []plumbing.Hash{commitIDs[4], commitIDs[3], commitIDs[2], commitIDs[1]}
	...
	t.Run("Get commits from non-existent commit", func(t *testing.T) {
		...
		assert.Nil(t, err)
		assert.Equal(t, commits, []plumbing.Hash{})
	})
}
...
func TestGetCommitsBetweenRangeForMergeCommits(t *testing.T) {
	...
	t.Run("Test commit 1", func(t *testing.T) {
		...
		assert.Nil(t, err)
		expectedCommits := []plumbing.Hash{commitIDs[0]}
		assert.Equal(t, expectedCommits, commits)
	})
	...
	t.Run("Test commit 2", func(t *testing.T) {
		...
		assert.Nil(t, err)
		expectedCommits := []plumbing.Hash{commitIDs[1], commitIDs[0]}
		assert.Equal(t, expectedCommits, commits)
	})
	...
	t.Run("Test commit 3", func(t *testing.T) {
		...
		assert.Nil(t, err)
		expectedCommits := []plumbing.Hash{commitIDs[0], commitIDs[2]}
		assert.Equal(t, expectedCommits, commits)
	})
	...
	t.Run("Test commit 4", func(t *testing.T) {
		...
		assert.Nil(t, err)
		expectedCommits := []plumbing.Hash{commitIDs[1], commitIDs[0], commitIDs[3]}
		assert.Equal(t, expectedCommits, commits)
	})
	...
	t.Run("Test commit 5, the merge commit", func(t *testing.T) {
		...
		commits, err := GetCommitsBetweenRange(repo, commitIDs[4], plumbing.ZeroHash)
		assert.Nil(t, err)
		expectedCommits := []plumbing.Hash{commitIDs[4], commitIDs[1], commitIDs[0], commitIDs[2]}
		assert.Equal(t, expectedCommits, commits)
	})
	...
	t.Run("Test commit 6", func(t *testing.T) {
		...
		assert.Nil(t, err)
		expectedCommits := []plumbing.Hash{commitIDs[0], commitIDs[5], commitIDs[2]}
		assert.Equal(t, expectedCommits, commits)
	})
}

func TestGetCommitsBetweenRangeUsingBinary(t *testing.T) {
	...
	t.Run("Check range between commits 1 and 5", func(t *testing.T) {
		commits, err := GetCommitsBetweenRangeUsingBinary(repo, commitIDs[4], commitIDs[0])
		assert.Nil(t, err)
		expectedCommits := []plumbing.Hash{commitIDs[4], commitIDs[3], commitIDs[2], commitIDs[1]}
		sort.Slice(expectedCommits, func(i, j int) bool {
			return expectedCommits[i].String() < expectedCommits[j].String()
	...
	t.Run("Pass in wrong order", func(t *testing.T) {
		...
		commits, err := GetCommitsBetweenRangeUsingBinary(repo, commitIDs[0], commitIDs[4])
		assert.Nil(t, err)
		assert.Empty(t, commits)
	})

	t.Run("Get all commits", func(t *testing.T) {
		commits, err := GetCommitsBetweenRangeUsingBinary(repo, commitIDs[4], plumbing.ZeroHash)
		assert.Nil(t, err)
		expectedCommits := commitIDs
		sort.Slice(expectedCommits, func(i, j int) bool {
			return expectedCommits[i].String() < expectedCommits[j].String()
	...
	t.Run("Get commits from invalid range", func(t *testing.T) {
		commits, err := GetCommitsBetweenRangeUsingBinary(repo, plumbing.ZeroHash, plumbing.ZeroHash)
		assert.NotNil(t, err)
		assert.Nil(t, commits)
	})
	...
	t.Run("Get commits from non-existent commit", func(t *testing.T) {
		nonExistentHash := EmptyBlob()
		commits, err := GetCommitsBetweenRangeUsingBinary(repo, nonExistentHash, plumbing.ZeroHash)
		assert.Nil(t, err)
		assert.Equal(t, commits, []plumbing.Hash{})
	})
}

func TestGetCommitsBetweenRangeUsingBinaryForMergeCommits(t *testing.T) {
	t.Setenv(dev.DevModeKey, "1")
	...
	t.Run("Test commit 1", func(t *testing.T) {
		// commit 1 is the first commit, so it should be the only commit returned
		commits, err := GetCommitsBetweenRangeUsingBinary(repo, commitIDs[0], plumbing.ZeroHash)
		assert.Nil(t, err)
		expectedCommits := []plumbing.Hash{commitIDs[0]}
		assert.Equal(t, expectedCommits, commits)
	})
	...
	t.Run("Test commit 2", func(t *testing.T) {
		// commit 2 is the first child of commit 1, so only it and commit 1 should be returned
		commits, err := GetCommitsBetweenRangeUsingBinary(repo, commitIDs[1], plumbing.ZeroHash)
		assert.Nil(t, err)
		expectedCommits := []plumbing.Hash{commitIDs[1], commitIDs[0]}
		assert.Equal(t, expectedCommits, commits)
	})
	...
	t.Run("Test commit 3", func(t *testing.T) {
		// commit 3 is the second child of commit 1, so only it and commit 1 should be returned
		commits, err := GetCommitsBetweenRangeUsingBinary(repo, commitIDs[2], plumbing.ZeroHash)
		assert.Nil(t, err)
		expectedCommits := []plumbing.Hash{commitIDs[0], commitIDs[2]}
		assert.Equal(t, expectedCommits, commits)
	})
	...
	t.Run("Test commit 4", func(t *testing.T) {
		// commit 4 is the child of commit 2, so only it, commit 2, and commit 2's parent commit 1 should be returned
		commits, err := GetCommitsBetweenRangeUsingBinary(repo, commitIDs[3], plumbing.ZeroHash)
		assert.Nil(t, err)
		expectedCommits := []plumbing.Hash{commitIDs[1], commitIDs[0], commitIDs[3]}
		assert.Equal(t, expectedCommits, commits)
	})
	...
	t.Run("Test commit 6", func(t *testing.T) {
		// commit 6 is the child of commit 3, so it should return commit 6, commit 3, and commit 1 (the parent of commit 3)
		commits, err := GetCommitsBetweenRangeUsingBinary(repo, commitIDs[5], plumbing.ZeroHash)
		assert.Nil(t, err)
		expectedCommits := []plumbing.Hash{commitIDs[0], commitIDs[5], commitIDs[2]}
		assert.Equal(t, expectedCommits, commits)
	})
