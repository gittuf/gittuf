...
both Git refs (tags and branches) and files tracked within the repository. 
Namespaces are combined with TUF's _delegations_ to define the set of actors 
who are authorized to view a namespace as well as those who are authorized to make
...
explicitly does not expect any Targets metadata to contain a target entry. Instead,
