...
with gittuf propagation, there is no upstream history tracking. TODO: we may
change this?
...
The subtree feature does not preserve the upstream repository's commit log. When
a subtree is updated with the remote's contents, a new commit is created in the
branch in the downstream repository that identifies the upstream repository's
commit ID that was copied as a subtree as well as the downstream repository's
branch tip.

* subtree is not automatically updated
* reference information is not tracked / is thrown away after subtree is created
* pulls down remote repo objects into local object store, subtree commit is a
  merge commit so gc doesn't clean it up => git log of embedded repo will work
  with some fiddling
...
TODO: benefit of having propagation recorded in the RSL
