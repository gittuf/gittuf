...
func runMetadataLs(cmd *cobra.Command, args []string) error {
	...
	currentTree, err := state.GetTreeForNamespace(gitstore.MetadataDir)
	if err != nil {
		return err
	}
...
func runMetadataAdd(cmd *cobra.Command, args []string) error {
	...
	return state.StageAndCommitMultipleMetadata(metadata)
}
...
func runMetadataCat(cmd *cobra.Command, args []string) error {
	...
	for _, n := range args {
		...
		contents, err := state.GetCurrentMetadataString(n)
		if err != nil {
			return err
		}
...
func runMetadataRm(cmd *cobra.Command, args []string) error {
	...
	return state.RemoveMetadata(roles)
}
